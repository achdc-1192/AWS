...
#
# Description: Check whether EC2 running instance is managed by SSM or not.
#
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    evaluation_list = []
    managed_instances = []
    ec2_resp = EC2_CLIENT.instances.filter(Filters=[{'Name': 'instance-state-name', 'Values': ['running']}])
    
    instance_association = SSM_CLIENT.describe_instance_information()
    ...
    for instances in ec2_resp:
        ...
        evaluation_list.append(build_evaluation(instances.instance_id, compliance_type, event))
